...
public class ElytraBotThreaded extends Module {
    ...
    private final Setting<Boolean> toggleOnPop = sgGeneral.add(new BoolSetting.Builder()
        .name("toggle-on-pop")
        .description("Whether to toggle the module if you pop a totem.")
        .defaultValue(false)
        .build()
    );
    ...
    public final Setting<Boolean> debug = sgGeneral.add(new BoolSetting.Builder()
        .name("debug")
        .description("Send debug messages to show what the bot is doing.")
        .defaultValue(false)
        .build()
    );
    ...
    public ElytraBotThreaded() {
        super(Categories.Movement, "elytra-bot", "Google maps for your elytra.");
    }
    ...
    public void onActivate() {
        ...
        // alternative to using packet event for checking if we popped
    ...
    public void loop() {
        ...
        // Switch low durability elytra with fresh one if setting on
        if (autoSwitch.get()) {
    ...
    public void generatePath() {
        ...
        if (botMode.get() == Mode.Highway) {
            ...
            //checkPositions = new ArrayList<BlockPos>(Arrays.asList(new BlockPos[]{new BlockPos(0, -1, 0)})); Janky way of doing it, below is cleaner and should
            // work the same
            checkPositions = new ArrayList<>(List.of(new BlockPos(0, -1, 0)));
        }

        if (path == null || path.size() == 0 || isNextPathTooFar() || Objects.requireNonNull(mc.player).isOnGround()) {
            BlockPos start;
            if (botMode.get() == Mode.Overworld) {
                start = getPlayerPos().add(0, 4, 0);
            } else {
                assert mc.player != null;
                if (Math.abs(jumpY - mc.player.getY()) <= 2) {
    ...
    private void walkTo(BlockPos goal) {
