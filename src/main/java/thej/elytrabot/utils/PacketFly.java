...
import thej.elytrabot.events.CancellablePlayerMoveEvent;
...
public class PacketFly {
    ...
    private void onMove(CancellablePlayerMoveEvent event) {
        ...
        Objects.requireNonNull(mc.getNetworkHandler()).sendPacket(new PlayerMoveC2SPacket.Full(mc.player.getX() + vel.x, mc.player.getY() + vel.y, mc.player.getZ() + vel.z, mc.player.getYaw(), mc.player.getPitch(), mc.player.isOnGround()));
    ...
    private void onPacketRecieve(PacketEvent.Receive event) {
        ...
        if (event.packet instanceof PlayerPositionLookS2CPacket packet && mc.currentScreen == null) {
            Objects.requireNonNull(mc.getNetworkHandler()).sendPacket(new TeleportConfirmC2SPacket(packet.getTeleportId()));
