...
import static thej.elytrabot.utils.MiscUtil.*;
...
public class AStar {
     ...
     * @positions The nearby positions that can possibly be added to the open list
    ...
    public static ArrayList<BlockPos> generatePath(BlockPos start, BlockPos goal, BlockPos[] positions, ArrayList<BlockPos> checkPositions, int loopAmount) {
        ...
        for (int i = 0; i < loopAmount; i++) {
            ...
            if (lowestFCost < fCost(closest, goal, start)) {
                ...
                //If there hasn't been a closer pos found in x times then break
                if (noClosest > 200) {
                    break;
                }
            }
    ...
    private static ArrayList<BlockPos> getPath(BlockPos current) {
        ...
        try {
            ...
            //Ignore if the path is zero in length
        }
